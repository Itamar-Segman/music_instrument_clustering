...
def preprocess_audio(file_path):
    ...
    resampled_audio = librosa.resample(audio, orig_sr=44100, target_sr=16000)
...
if __name__ == '__main__':
    ...
    inputs = processor(dataset, sampling_rate=16000, return_tensors="pt")
    ...
    kmeans = KMeans(n_clusters=4, random_state=0, n_init="auto").fit(time_reduced_hidden_states)

    clustering = SpectralClustering(n_clusters=4,
                                    assign_labels='discretize', random_state=0).fit(time_reduced_hidden_states)

    opc = OPTICS(min_samples=2).fit(time_reduced_hidden_states)
    ...
    plt.legend()
    plt.title('Embedding using MERT-v0-public model')
    plt.show()
